...
import { Convenio, StatusConvenio } from './entities/convenio.entity';
...
export class ConveniosService {
  ...
  async update(
    ...
    if (
      ...
      updateConvenioDto.codigo !== convenio.codigo_convenio
    ) {
      const existingConvenio = await this.convenioRepository.findOne({
        where: { codigo_convenio: updateConvenioDto.codigo },
      });
    ...
    // Se está alterando o CNPJ, verifica duplicação através da empresa
    if (
      updateConvenioDto.cnpj &&
      convenio.empresa &&
      updateConvenioDto.cnpj !== convenio.empresa.cnpj
    ) {
      const existingCnpj = await this.convenioRepository.findOne({
        where: { empresa: { cnpj: updateConvenioDto.cnpj } },
        relations: ['empresa'],
      });
  ...
  async findAtivos(): Promise<Convenio[]> {
    return await this.convenioRepository.find({
      where: { status: StatusConvenio.ATIVO },
      order: { nome: 'ASC' },
    });
  }
  ...
  async findComIntegracao(): Promise<Convenio[]> {
    return await this.convenioRepository.find({
      where: { tem_integracao_api: true, status: StatusConvenio.ATIVO },
      order: { nome: 'ASC' },
    });
  }
  ...
  async findByTipoFaturamento(tipo: string): Promise<Convenio[]> {
    return await this.convenioRepository.find({
      where: { tipo_faturamento: tipo as any, status: StatusConvenio.ATIVO },
      order: { nome: 'ASC' },
    });
  }
